import { readableDate } from './format.js'
...
function EveningSummary({ evening, availability }) {
  if (!availability.cinema && !availability.food) {
    return <span>Provider details are no longer retained.</span>
  }
  const cinema = availability.cinema
    ? `${readableDate(evening.cinema.date)} · ${evening.cinema.time}`
    : 'Film details expired'
  const food = availability.food ? evening.food.name : 'Dinner details expired'
  return <span>{cinema} · {food}</span>
}

function SavedEveningCard({ evening, onSelect }) {
  const availability = savedEveningAvailability(evening)
  return (
    <button type="button" className="ios-card ios-saved-card" onClick={() => onSelect(evening)}>
      <span className="ios-card-kicker">{evening.status === 'completed' ? 'COMPLETED' : 'SAVED EVENING'}</span>
    ...
    <main className="ios-page ios-detail-page">
      ...
      {availability.cinema ? (
        <section className="ios-saved-detail-block">
          <h2>Film</h2>
          <p>{readableDate(evening.cinema.date)} · {evening.cinema.time} · {evening.cinema.format}</p>
